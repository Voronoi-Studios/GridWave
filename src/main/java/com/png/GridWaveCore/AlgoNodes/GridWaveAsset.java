...
public class GridWaveAsset extends PropDistributionAsset {
    ...
    private static final ConcurrentHashMap<String, AtomicReference<List<GridWave.GridTile>>> winnerGridTilesMap = new ConcurrentHashMap<>();
    ...
    public PropDistribution build(@Nonnull Argument argument) {
        ...
        if (super.isSkipped()) {
            ...
            AtomicReference<List<GridWave.GridTile>> winnerGridTiles = winnerGridTilesMap.computeIfAbsent(seedBox.toString(), k -> new AtomicReference<>());
            ...
            var wfcWave = GridWave.performWFC(baseWave, grid,multithreading ? seedBox.child(workerId + "w") : seedBox, this.maxAttempts, this.maxBacktracks, this.debug, winner);
            var fancyWave = GridWave.placeFancyTiles(wfcWave, fancyTileEntries,  seedBox.child("fancy"), winner);
            List<GridWave.GridTile> gridTiles;
            if (multithreading) {
                if (winner.get() == -1 && winnerGridTiles.compareAndSet(null, new ArrayList<>(fancyWave.values().stream().map(GridWave.WaveCell::getChosen).toList()))) {
                    winner.set(workerId);
                }
                gridTiles = new ArrayList<>(winnerGridTiles.get());
            }
            else {
                gridTiles = new ArrayList<>(fancyWave.values().stream().map(GridWave.WaveCell::getChosen).toList());
            }

            if (workerId == 1) {
                String generatedString = GridWave.generateString(gridTiles, pathKey);
